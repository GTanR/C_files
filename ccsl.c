#include <stdio.h>
#include <stdlib.h>
void str_comp(char s1[], char s2[]); //function declarations
 ...
 void str_concat(char s1[50], char s2[50])
{
    int i, j;
    for (i = 0; s1[i] != '\0'; i++)
        ; // to get the last position of str1 to concatenate str2 from that position.
    for (j = 0; s2[j] != '\0'; j++, i++)
    {
        s1[i] = s2[j];
    }
...
void str_length(char s1[50])
{
    int i;
    for (i = 0; s1[i] != '\0'; i++)
        ; // to get the last position of str1, which is its length
